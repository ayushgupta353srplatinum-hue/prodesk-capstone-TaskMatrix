...
function Dashboard() {
  ...
  const handleAiSuggest = async () => {
    ...
    if (title.length < 3) return toast.error("AI suggestion ke liye title bada likho!");
    ...
    try {
      ...
      if (result.success) {
        ...
        toast.error(result.details || "AI failed!");
      }
    } catch {
  ...
  return (
    <>
      <style>{`
        ...
        .sidebar { width: 260px; background: #000; padding: 30px; display: flex; flexDirection: column; justify-content: space-between; position: fixed; height: 100vh; }
        .main-content { flex: 1; background: #f9f9f9; padding: 30px; margin-left: 260px; }
        .nav-item.active { color: #facc15; padding: 10px 0; cursor: pointer; font-weight: bold; }
        .nav-badge { background: #333; color: #facc15; padding: 2px 8px; border-radius: 10px; font-size: 12px; margin-left: 10px; }
      ...
      Logout
        ...
        <main className="main-content">
          <div className="top-bar" style={{ display: 'flex', gap: '15px', marginBottom: '30px' }}>
            ...
            <button onClick={handleAiSuggest} disabled={isAiLoading} style={{ background: '#6366f1', color: 'white', padding: '10px 20px', borderRadius: '30px', fontWeight: 'bold', border: 'none', cursor: 'pointer' }}>
              {isAiLoading ? "Thinking..." : "✨ AI Suggest"}
            </button>

            <button onClick={() => addTask()} disabled={isAdding} style={{ background: '#000', color: '#facc15', padding: '10px 25px', borderRadius: '30px', fontWeight: 'bold', border: 'none', cursor: 'pointer' }}>
